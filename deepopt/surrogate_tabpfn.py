...
class TabPFN(Model):
    """
    The `TabPFN` class represents a tabpfn neural net transformer. This model will allow us to set the training data, fit it, and get
    ...
    def __init__(
        ...
        assert self.output_dim==1, 'TabPFN currently only supports regression on 1 scalar output.'
    ...
    def forward(self, X: torch.Tensor, **kwargs) -> MultivariateNormal:
        ...
        """
        use_variances = kwargs.get("use_variances",True)
    ...
    def get_prediction_with_uncertainty(
        ...
        **kwargs: Any,
        ...
        if original_scale:
            if self.multi_fidelity:
                mu_new, var_new = torch.zeros_like(mu), torch.zeros_like(var)
                for fid_loc, ymin, ymax in zip(test_fid_locs,self.y_min,self.y_max):
                    mu_new[fid_loc] = self.out_scaler_inv(mu[fid_loc],ymin,ymax)
                    var_new[fid_loc] = var[fid_loc]*(ymax-ymin)**2
                mu, var = mu_new, var_new
            else:
                mu = self.out_scaler_inv(mu,self.y_min,self.y_max)
                var*=(self.y_max-self.y_min)**2

        # mu = all_preds.mean(axis=0)
        # var = all_preds.var(axis=0)
        if get_cov:
            print('Covariances not yet available with TabPFN.')
            return
