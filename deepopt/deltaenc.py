...
from .surrogate_utils import create_optimizer
from .surrogate_utils import MLP as Arch
...
class DeltaEnc(BaseModel):
    ...
    def fantasize(self,X,sampler,observation_noise=True,**kwargs):
        ...
        X_train = torch.cat([self.X_train.expand(*X_train_new.shape[:-2],*self.X_train.shape[-2:]),X_train_new],axis=-2)
        Y_train = torch.cat([self.y_train.expand(*Y_fantasized.shape[:-2],*self.y_train.shape[-2:]),Y_fantasized],axis=-2)
