...
1. **Learn:** The `learn` command trains a surrogate model on a given set of data. Users can select `GP`, `delUQ`, or `nnEnsemble` models. Uncertainty quantification is available in all models, allowing direct use in a Bayesian optimization workflow. The `learn` command supports multi-fidelity modeling with an arbitrary number of fidelities.
